...
def affine_forward(x, w, b):
  ...
  #############################################################################    
  global x_origshape 
  x_origshape = np.asarray(x.shape)
  #print len(x_origshape)
  if len(x_origshape) > 2:
    one_to_k = np.prod(x.shape[1:])
    x = x.reshape( [x.shape[0], one_to_k] )
  #print x.shape
  out = np.dot(x,w) + b
  #############################################################################
  #                             END OF YOUR CODE                              #
...
def affine_backward(dout, cache):
  ...
  #############################################################################
  # print np.asarray(cache[2]).shape
  dx = np.dot(dout,w.T)
#  print "dx before reshape:", dx.shape
#  print "x_origshape:", x_origshape
  if len(x_origshape) > 2:
    dx = dx.reshape(x.shape[0],x_origshape[1],x_origshape[2])
 
#  print "dx after reshape: ", dx.shape
  dw = np.dot(x.T,dout)
...
#  print x.shape
  ...
  if mode == 'train':
    ...
    #############################################################################
    # Implementing Algorithm 1 given in https://arxiv.org/pdf/1502.03167v3.pdf
    ...
    cache = (gamma, beta, sample_var, sample_mean, xhat, x , eps)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
  elif mode == 'test':
...
def batchnorm_backward(dout, cache):
  ...
  dxhat = dout*gamma
  dvar   = -0.5*np.sum( dxhat*(x - sample_mean)*np.power( (sample_var + eps) , -1.5), axis=0)
  dmean  = -1*np.sum( dxhat/np.power(sample_var + eps , 0.5), axis = 0)
  dx     = dxhat*np.power(sample_var + eps , -0.5)
