...
#    self.params['W1'] = self.params['W1'] * (np.sqrt(2.0/hidden_dim/num_classes))
    self.params['b1'] = np.zeros(hidden_dim)
    self.params['W2'] = np.random.normal(0,weight_scale,[hidden_dim,num_classes])
#    self.params['W2'] = self.params['W2'] * (np.sqrt(2.0/hidden_dim/num_classes)) 
  ...
  def loss(self, X, y=None):
    ...
    # As suggested in FullyConnectedNet function below here is the implemented 
    # arc without dropout and batch normalization:
    # affine:
    out1, cache1 = affine_forward(X, self.params['W1'],self.params['b1'])
    # relu:
    out2, cache2 = relu_forward(out1)
    # affine-relu 
    #out12, cache12 = affine_relu_forward(X, self.params['W1'],self.params['b1'])
    # affine:
    ...
    dx2, grads['W2'], grads["b2"] = affine_backward(dout, cache3)
    relu_dx2 = relu_backward(dx2, cache2)
    # dx1, dx2, db1 = affine_relu_backward(dout, cache2)
    dx1, grads['W1'], grads["b1"] = affine_backward(relu_dx2, cache1)
    l = self.reg * (np.sum(self.params['W1']*self.params['W1']) + \
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers):
        l1 += .5*self.reg*np.sum(self.params['W'+str(i+1)]*self.params['W'+str(i+1)])
    l2, dscores = softmax_loss(scores, y)
    loss = l1 + l2

    dx, grads['W'+str(self.num_layers)], grads['b'+str(self.num_layers)] = affine_backward(dscores, aff_cache)
